...
const updateServerFile = (baseName)=>{
  const serverPath = './server.js';
  let content = fs.readFileSync(serverPath, 'utf-8');

  const importLine = `import ${baseName}Routes from './routes/${baseName}.route.js';`;
  const useLine = `app.use('/${baseName}', ${baseName}Routes);`;

  // === Add import ===
  if (!content.includes(importLine)) {
    const lines = content.split('\n');
    // Find imports last line 
    const lastImportIndex = lines.reduce((last, line, i) => {
      if (line.startsWith('import ')) return i;
      return last;
    }, -1);
  ...
  // === Add routes section (end of file) ===
  ...
  console.log(`✅ Routes "${baseName}" ajoutées dans server.js`);
}


export {generateSchemaFields, generateValidationFieldsForCreate, generateValidationFieldsForUpdate, check, capitalize, updateServerFile}